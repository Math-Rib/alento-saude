...
function applySavedTheme() {
  if (localStorage.getItem("alento-theme") === "dark") {
    body.classList.add("dark-mode");
    if (darkModeButton) darkModeButton.style.backgroundColor = "#5D99C6";
  }
}
...
function toggleDarkMode() {
  ...
  localStorage.setItem("alento-theme", isDark ? "dark" : "light");
  if (darkModeButton) {
    darkModeButton.style.backgroundColor = isDark ? "#5D99C6" : "#ccc";
  }
}
...
modalAjuda?.addEventListener("click", e => {
  if (e.target === modalAjuda) closeHelpModal();
});

fecharPerfil?.addEventListener("click", closePerfilModal);
modalPerfil?.addEventListener("click", e => {
  if (e.target === modalPerfil) closePerfilModal();
});
